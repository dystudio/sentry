...
class BigtableNodeStorage(NodeStorage):
    ...
    def get_multi(self, id_list):
        ...
        return {
            id: future.result()
            for id, future in izip(
                id_list,
                (
                    self.thread_pool.submit(self.get, id)
                    for id in id_list
                )
            )
        }
    ...
    def delete_multi(self, id_list):
        if len(id_list) == 1:
            return self.delete(id_list[0])

        for future in (self.thread_pool.submit(self.delete, id) for id in id_list):
            future.result()
